---
title: "Compare the ZooScat DWBA to an analytical solution"
...
vignette: >
  %\VignetteIndexEntry{Compare the ZooScat DWBA to an analytical solution}
  %\VignetteEngine{knitr::rmarkdown}
  %\usepackage[utf8]{inputenc}
---
This document compares the results of the the DWBA implementation in ZooScat to an anlytical scattering model of a sphere [@anderson_sound_1950-2], following routines described in Jech et al. [-@jech_comparisons_2015].

This vignette requires the ZooScat package for the implementation of the DWBA model, ggplot2 for plotting, viridis for nicer colors.
...
For the purpose of this example we will build the general anlytical model for a scattered wave by a sphere [@anderson_sound_1950-2]:  
...
* $A_n$ is defined by the boundary conditions [@anderson_sound_1950-2]. For $A_n$, definitions following [@jech_comparisons_2015] were used, rather than [@anderson_sound_1950-2].  
...
* $\theta$ is the scattering angle  
  ...
  In the farfield $k_wr \gg 1, h^1_n(k_wr) \xrightarrow[k_wr\gg1]{} \frac{[(-i)^{n+1}]}{(k_wr)}e^{ik_wr} $  the acoustic backscatter pressure $p_{bs}$ can be expressed as a function of backscattering amplitude ($f_{bs}$, incident pressure, phase and range:  
  ...
  From $f_bs$, target strength $TS$ in $dB \text{ re }m^2$ can be computed as [@anderson_sound_1950-2]:  
  $$TS = 10log_{10}(|f_bs|^2)$$  
  ...
  * $y$ is the spherical Neumann function  
  ...
  These functions have been implemented within ZooScat and can be called using:  
...
Set the settings inside ZooScat:  
...
para$simu$ni=1000 #any high number

```

## Building a sphere inside ZooScat
...
```  
   ...
   The ZooScat DWBA model performs well for the material preperties of krill, when compared to the analytical model.
...
```  
   ...
   The ZooScat DWBA model holds no validity for such strong scatterers.
##References
