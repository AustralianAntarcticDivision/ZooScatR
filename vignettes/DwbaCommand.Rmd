...
set.seed(1014)
```
This vignette will provide a brief description of the DWBA model used within the ZooScat package, explain the needed input parameters and provide a few examples.  
...
$$f_{bs} = \frac{k_s}{4}\int_{\vec{r_{pos}}}a_c(\gamma_k - \gamma_{\rho})e^{2i\vec{k_b}\vec{r_{pos}}}\frac{J_1(2k_ba_{c}cos\beta_{tilt})}{cos\beta_{tilt}} $$
where:  
...
* $\gamma_k$ and $\gamma_{\rho}$ are related to the body material properties $g$ - the density contrast and $h$ the soundspeed contrast. Both contrast are the ratio of the density ($\frac{\rho_b}{\rho_s}$) or soundspeed ($\frac{c_b}{c_s}$) inside the body and the surrounding fluid.  
    + $\gamma_k = \frac{(1 - gh2)}{gh2}$ is the compressibility  

    + $\gamma_{\rho} = \frac{(g - 1)}{g}$  is related to the density contrast 
The term $\gamma_{\kappa} - \gamma_{\rho}$ is equal to the plane wave / plane interface reflection coefficient ($\Re_{s,b}$)where the wave is initially in medium $s$ and incident upon medium $b$:  
$$\Re_{s,b}=\frac{\rho_} $$
...
## Loading parameters into ZooScat
  
Before the DWBA model can be run several parameters have to be set.  
Some example configuration files containing all parameters can be found in: `r  paste0(system.file(package="ZooScat"),"/extdata/configs")`
Parameters (config files) can be loaded into ZooScat:
```{r}
...
ZooScat includes routines to compute the soundspeed of the surrounding water following 3 methods:  
